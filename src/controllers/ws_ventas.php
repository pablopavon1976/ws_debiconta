...
include_once ('src/funciones/igtech.funciones_cierre_caja.php');

$app->post('/ventas/nuevo',function(Request $request,Response $response){
    ...
    try{
        ...
        // if($w_datos_token->respuesta ==0 and $w_autorizacion['datos']['estado']=='V'){
            $o_respuesta=registrarNuevaVenta($w_data);
...
$app->post('/cierre_caja',function(Request $request,Response $response){
    //$i_token=$request->getHeaderLine('Authorization');
    //$i_ct=$request->getHeaderLine('Content-Type');
    $i_accept=$request->getHeaderLine('Accept');
    $i_body = $request->getBody();
    ...
    try{
        ...
        // $w_autorizacion= seleccionarToken($i_token);
        // if($w_datos_token->respuesta ==0 and $w_autorizacion['datos']['estado']=='V'){
            $o_respuesta=registrarNuevoCierreCaja($w_data);
