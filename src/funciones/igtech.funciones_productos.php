<?php
function listaProductos($i_empresa){
    try{
        $Log=new IgtechLog ();
        $Log->Abrir();
        ...
        $select_sql="SELECT        ID_GRUPOS_PRODUCTOS_VENTAS, NOMBRE
...
function BuscaProducto($i_producto){
    try{
        $Log=new IgtechLog ();
        $Log->Abrir();
        $Log->EscribirLog(' LISTA ');
        ...
        $select_sql="SELECT        ID_IMPUESTO, CUENTA
                     FROM        GRUPO_PRODUCTO_VENTA
                    WHERE        (ID_GRUPOS_PRODUCTOS_VENTAS = $i_producto)";
        $Log->EscribirLog(' Consulta: '.$select_sql);
        $result = sqlsrv_query($ws_conexion, $select_sql);
        if($result===false){
            $o_respuesta=array('error'=>'9997','mensaje'=>sqlsrv_errors());
...
function listaProductosCompra($i_empresa){
    try{
        ...
        if($result===false){
            ...
            while($row = sqlsrv_fetch_object( $result)) { 
                $w_respuesta[] = array(
                    'ID_GRUPOS_PRODUCTOS_COMPRAS' =>  utf8_encode($row->IDPRODCOM),
                    'NOMBRE'      =>  utf8_encode($row->NOMBRE)
                );
            }
            $o_respuesta=array('error'=>'0','mensaje'=>'ok','datos'=>$w_respuesta);
